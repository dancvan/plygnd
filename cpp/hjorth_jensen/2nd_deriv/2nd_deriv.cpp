...
#include <fstream>
#include <iomanip>
// output file as a global variable
std::ofstream ofile;
...
int main(int argc, char* argv[])
{
    ...
    // Read in output file, abour if there are too few command-line arguments
    if( argc <= 3 ){
	std::cout << "Bad Usage: " << argv[0]  << " read also output file, number of integration points and the final x values on same line, four variables in total" << std::endl;
	exit(1);
    }
    else{
	outfilename=argv[1];
